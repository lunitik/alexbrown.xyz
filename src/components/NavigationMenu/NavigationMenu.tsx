...
import { Box, Divider, Drawer, IconButton, List, ListItem, Toolbar, Typography, useTheme } from "@mui/material";
...
function NavigationMenu() {
    ...
    const drawer = (
        <Box onClick={handleDrawerToggle} sx={{ textAlign: 'center' }}>
            ...
            {navItems.map((item) => (
                <ListItem key={item.type} alignItems="center" disablePadding>
                    {item}
                </ListItem>
            ))}
            </List>
        ...
        <Suspense fallback="loading">
            <nav className={colourModeClass}>
                ...
                <Toolbar>
                    ...
                    <Box className="navigationmenu__container-utility" sx={{ display: { xs: 'none', sm: 'flex' } }}>
                        {navItems.map((item) => (
                            item
                        ))}
                    </Box>
